...
encrypted_credentials = node["monit"]["web_interface"]["encrypted_credentials"]
if encrypted_credentials
  bag_name = node["monit"]["web_interface"]["encrypted_credentials_data_bag"]
  credentials = Chef::EncryptedDataBagItem.load(bag_name, encrypted_credentials)

  admin_username = credentials["username"]
  admin_password = credentials["password"]
  node.override["monit"]["web_interface"]["allow"] = ["0.0.0.0/0.0.0.0", "#{admin_username}:#{admin_password}"]

  Chef::Log.info "Using encrpyted admin dashboard credentials: #{encrypted_credentials}"
end
